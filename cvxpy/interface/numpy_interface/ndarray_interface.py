...
class NDArrayInterface(base.BaseMatrixInterface):
    """
    An interface to convert constant values to the np.ndarray class.
    """
    ...
    def const_to_matrix(self, value, convert_scalars: bool = False):
        ...
        if sp.issparse(value):
            ...
            result = np.asarray(value).T
        else:
            result = np.asarray(value)
        if result.dtype in [np.float64] + COMPLEX_TYPES:
            return result
        else:
    ...
    def scalar_matrix(self, value, shape: Tuple[int, ...]):
        return np.zeros(shape, dtype='float64') + value
