...
class quad_over_lin(AxisAtom):
    ...
    def __init__(
        self, x, y, axis: int | tuple[int, ...] | None = None, keepdims: bool = False
    ) -> None:
        self.axis = axis
    ...
    def name(self) -> str:
        """Returns a clean string representation of the quad_over_lin atom."""
        x_name = self.args[0].name()
        y_name = self.args[1].name()
        ...
        if params:
            return f"quad_over_lin({x_name}, {y_name}, {', '.join(params)})"
        return f"quad_over_lin({x_name}, {y_name})"
