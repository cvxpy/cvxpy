        ...
        if self.axis == None:
            ...
            if self.axis == 0: # function apply to each column
                ...
                for i in range(n):
                    ...
                    col = np.ones((m))*i
                    D = D + scipy.sparse.csc_matrix((np.array(d)[0],(row,col)),shape = (m*n,n)) # d must be 1-D
            else: # function apply to each row
                values = np.transpose(values[0])
                D = scipy.sparse.csc_matrix((m*n,m),dtype = np.float)
                for i in range(m):
                    value = values[:,i]
                    d = self._column_grad(value).T
                    row = np.linspace(i, i+(n-1)*m, n) # [0+i, m+i, ..., m(n-1)+i]
                    col = np.ones((n))*i
                    D = D + scipy.sparse.csc_matrix((np.array(d)[0],(row,col)),shape = (m*n,m)) # d must be 1-D
        return [D]
