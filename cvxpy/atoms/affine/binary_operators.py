...
class BinaryOperator(AffAtom):
    ...
    def is_complex(self) -> bool:
        ...
        return (self.args[0].is_complex() or self.args[1].is_complex()) and not (
            self.args[0].is_imag() and self.args[1].is_imag()
        )
...
class MulExpression(BinaryOperator):
    ...
    def numeric(self, values):
        ...
        if (
            values[0].shape == ()
            or values[1].shape == ()
            or intf.is_sparse(values[0])
            or intf.is_sparse(values[1])
        ):
    ...
    def is_atom_convex(self) -> bool:
        ...
        if u.scopes.dpp_scope_active():
            ...
            y = self.args[1]
            return (
                (x.is_constant() or y.is_constant())
...
class multiply(MulExpression):
    ...
    def is_atom_quasiconvex(self) -> bool:
        return (
            (self.args[0].is_constant() or self.args[1].is_constant())
            or (self.args[0].is_nonneg() and self.args[1].is_nonpos())
            or (self.args[0].is_nonpos() and self.args[1].is_nonneg())
        )

    def is_atom_quasiconcave(self) -> bool:
        return (
            (self.args[0].is_constant() or self.args[1].is_constant())
            or all(arg.is_nonneg() for arg in self.args)
            or all(arg.is_nonpos() for arg in self.args)
        )
