...
class moveaxis(AffAtom):
    """
    Move source axes of an array to destination positions, 
    keeping the order of the other axes.
    """

    def __init__(self, expr, source, destination) -> None:
        self.source = source
        self.destination = destination
        super(moveaxis, self).__init__(expr)
    ...
    def numeric(self, values) -> Optional[np.ndarray]:
        return np.moveaxis(values[0], source=self.source, destination=self.destination)
