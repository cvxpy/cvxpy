...
class gen_lambda_max(Atom):
    ...
    def numeric(self, values):
        ...
        return LA.eigh(a=values[0], b=values[1], eigvals_only=True, subset_by_index=(lo, hi))[0]

    def _domain(self) -> List[Constraint]:
        """Returns constraints describing the domain of the node."""
        return [self.args[0].H == self.args[0], self.args[1].H == self.args[1], self.args[1] >> 0]
    ...
    def validate_arguments(self) -> None:
        ...
        if (
            not self.args[0].ndim == 2
            or self.args[0].shape[0] != self.args[0].shape[1]
            or self.args[1].shape[0] != self.args[1].shape[1]
            or self.args[0].shape != self.args[1].shape
        ):
            raise ValueError(
