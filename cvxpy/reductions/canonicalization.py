        ...
        if type(expr) == cvxtypes.partial_problem():
            ...
            for constr in expr.args[0].constraints:
                canon_constr, aux_constr = self.canonicalize_tree(
                    constr, canonicalize_params=canonicalize_params
                )
            ...
            for arg in expr.args:
                canon_arg, c = self.canonicalize_tree(arg, canonicalize_params=canonicalize_params)
                canon_args += [canon_arg]
                constrs += c
            canon_expr, c = self.canonicalize_expr(
                expr, canon_args, canonicalize_params=canonicalize_params
            )
            constrs += c
