...
class DualizeConeProg(Reduction):
    ...
    2. Returning a ``Solution`` whose ``primal_vars`` and
       ``dual_vars`` follow the ``Dualize`` convention so that
       ``invert`` can recover the original solution.
