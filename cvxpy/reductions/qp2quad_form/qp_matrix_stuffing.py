...
class QpMatrixStuffing(MatrixStuffing):
    ...
    def apply(self, problem):
        ...
        ##################################################
        # Begin monkey patch logic
        import os
        import pandas as pd

        # Keep a mapping of the constraints in the final reducer of the solver chain
        s.LOGGER.info("Monkey patch logic started...")
        default_filename = "~/constraint_mapping.pq"
        if "constraint_mapping_filename" not in os.environ.keys():
            s.LOGGER.warning(
                f'"constraint_mapping_filename" environment varialbe not found, using default filename: {default_filename}'
            )
        constraint_mapping_filename = os.environ.get(
            "constraint_mapping_filename", default_filename
        )

        row_offset = 0
        constraint_mapping = []
        lin_ops = [e.canonical_form[0] for e in expr_list]
        for c, lin_op in dict(zip(ordered_cons, lin_ops)).items():
            lin_op_rows = np.prod(lin_op.shape)
            constraint_map = dict()
            constraint_map["ConstraintID"] = c.id
            constraint_map["LinOp"] = str(
                lin_op
            )  # Pandas can't serialize complext objects, use string repr
            constraint_map["OffsetStart"] = row_offset
            constraint_map["OffsetEnd"] = row_offset + lin_op_rows
            constraint_mapping.append(constraint_map)
            row_offset += lin_op_rows

        xf = pd.DataFrame(constraint_mapping)
        xf.to_parquet(constraint_mapping_filename)
        del os
        del pd
        s.LOGGER.info("Monkey patch logic completed...")
        # End monkey patch logic
        ##################################################
