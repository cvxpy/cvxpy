...
class DAQP(QpSolver):
    ...
    def solve_via_data(self, data, warm_start: bool, verbose: bool, solver_opts, solver_cache=None):
        ...
        A = np.array(np.concatenate([data[s.A].todense(), data[s.F].todense()]), dtype=c_double)
        ...
        bupper = np.array(np.concatenate((var_upper_bounds, data[s.B], data[s.G])), dtype=c_double)
        ...
        blower = np.array(
            np.concatenate((var_lower_bounds, data[s.B], -np.inf * np.ones(data[s.G].shape))),
            dtype=c_double,
        )
        ...
        used_solver_opts = {k: solver_opts[k] for k in solver_opts if k in self.ALLOWED_SOLVER_OPTS}
