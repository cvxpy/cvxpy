...
class CUOPT(QpSolver):
    """QP interface for the cuOpt solver"""

    # Solver capabilities.
    MIP_CAPABLE = True
    SUPPORTED_CONSTRAINTS = QpSolver.SUPPORTED_CONSTRAINTS
    ...
    def solve_via_data(self, data, warm_start: bool, verbose: bool, solver_opts, solver_cache=None):
        ...
        Acsr = sp.vstack([data[s.A], data[s.F]]).tocsr()
        #data['Ax'] = A
        upper_bounds = np.concatenate((data[s.B], data[s.G]))
        #data['u'] = uA
        lower_bounds = np.concatenate([data[s.B], -np.inf*np.ones(data[s.G].shape)])
        #data['l'] = lA
        leq_start = len(data[s.B])
        leq_end = len(lower_bounds)

        Qcsr = data[s.P].tocsr()/2
        num_vars = data['q'].shape[0]
        ...
        data_model.set_objective_coefficients(data['q'])
