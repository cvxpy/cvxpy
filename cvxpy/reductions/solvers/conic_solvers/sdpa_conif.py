...
class SDPA(ConicSolver):
    ...
    def solve_via_data(self, data, warm_start: bool, verbose: bool, solver_opts, solver_cache=None):
        ...
        # BUG FIX: In `sdpa` (not `sdpa-python`) source, there's a `#define REVERSE_PRIMAL_DUAL`
        # By default it's enabled.
        # The function `SDPA::PhaseType SDPA::getPhaseValue()`, also in `sdpa` source
        # should check if it's enabled and if yes, it should flip twice (or not flip)
        # the statuses highlighted below

        # Why twice? Currently the primal of `sdpa-python` always becomes the dual in `sdpa`
        # because the only conversion available is `clp_toLMI` (see long note in `apply` method)
        # First flip: because of `clp_toLMI`
        # Second flip: because of `#define REVERSE_PRIMAL_DUAL`
        # `sdpa-python` calls `getPhaseValue` directly and hence, only one flip happens
        # We need two flips (or no flip)
        ...
        BUG_FIX = {
            ...
            "pdFEAS": "pdFEAS",
            "pdINF": "pdINF",
            "pFEAS_dINF": "pINF_dFEAS",  # (second) flip
            "pINF_dFEAS": "pFEAS_dINF",  # (second) flip
            "pdOPT": "pdOPT",
            "pUNBD": "dUNBD",  # (second) flip
            "dUNBD": "pUNBD"  # (second) flip
        }

        if (True):  # set to False if you disable `#define REVERSE_PRIMAL_DUAL`
            sdpainfo['phasevalue'] = BUG_FIX[sdpainfo['phasevalue']]
