...
class CanonBackend(ABC):
    ...
    def get_backend(cls, backend_name: str, *args, **kwargs) -> CanonBackend:
        ...
        from cvxpy.lin_ops.backends.numpy_backend import NumPyCanonBackend
...
# Re-export concrete backends for backwards compatibility
# These are imported lazily to avoid circular imports
def __getattr__(name):
    if name == "NumPyCanonBackend":
        from cvxpy.lin_ops.backends.numpy_backend import NumPyCanonBackend
        return NumPyCanonBackend
    elif name == "NumPyTensorView":
        from cvxpy.lin_ops.backends.numpy_backend import NumPyTensorView
        return NumPyTensorView
    elif name == "SciPyCanonBackend":
        from cvxpy.lin_ops.backends.scipy_backend import SciPyCanonBackend
        return SciPyCanonBackend
    elif name == "SciPyTensorView":
        from cvxpy.lin_ops.backends.scipy_backend import SciPyTensorView
