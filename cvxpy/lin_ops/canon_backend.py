...
class PythonCanonBackend(CanonBackend):
    ...
    def get_constant_data(self, lin_op: LinOp, view: TensorView, column: bool) \
            -> tuple[np.ndarray | sp.sparray, bool]:
        """
    ...
    @staticmethod
    @abstractmethod
    def get_constant_data_from_const(lin_op: LinOp) -> np.ndarray | sp.sparray:
        """
...
class SciPyCanonBackend(PythonCanonBackend):
    ...
    def sum_entries(self, _lin: LinOp, view: SciPyTensorView) -> SciPyTensorView:
        ...
        def func(x, p):
            ...
            if p == 1:
                if axis is None:
                    return sp.csr_array(x.sum(axis=0))
                else:
...
class SciPyTensorView(DictTensorView):
    ...
    def apply_to_parameters(self, func: Callable,
                            parameter_representation: dict[int, sp.sparray]) \
            -> dict[int, sp.sparray]:
        """
    ...
    @staticmethod
    def add_tensors(a: sp.sparray, b: sp.sparray) -> sp.sparray:
        """
