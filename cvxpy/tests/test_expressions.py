...
class TestND_Expressions():
    ...
    def test_nd_matmul_rmul_not_implemented(self) -> None:
    ...
    def test_nd_matmul_mul(self) -> None:
        """Test ND matmul: Constant (m,k) @ Variable (B,k,n)."""
        B, m, k, n = 2, 3, 4, 5
        ...
        # Solve with a feasible target and verify the result
        target = np.random.randn(B, m, n)
        prob = cp.Problem(cp.Minimize(cp.sum_squares(expr - target)))
        prob.solve()
        assert prob.status == cp.OPTIMAL
        # Verify the matmul produces the expected shape and values
        result = C @ X.value
        assert result.shape == (B, m, n)
        # Since the problem is well-posed (k >= m for this case), error should be small
        error = np.linalg.norm(result - target) / np.linalg.norm(target)
        assert error < 1e-4, f"Relative error {error} too large"

    def test_nd_matmul_higher_dim(self) -> None:
        """Test ND matmul with more than 3 dimensions: C @ X where both have >2 dims."""
        B1, B2, m, k, n = 2, 3, 4, 5, 6
        C = np.random.randn(B1, B2, m, k)
        ...
        for i in range(B1):
            for j in range(B2):
                result[i, j] = C[i, j] @ X.value[i, j]
        error = np.linalg.norm(result - target) / np.linalg.norm(target)
        assert error < 1e-4, f"Relative error {error} too large"

    def test_nd_matmul_both_backends(self) -> None:
        """Test ND matmul works with both SCIPY and COO backends."""
        B, m, k, n = 2, 3, 4, 5
        C = np.random.randn(m, k)
        ...
        for backend in [cp.SCIPY_CANON_BACKEND, cp.COO_CANON_BACKEND]:
            ...
            result = C @ X.value
            error = np.linalg.norm(result - target) / np.linalg.norm(target)
            assert error < 1e-4, f"Backend {backend}: relative error {error} too large"

    def test_nd_matmul_batch_varying_constant(self) -> None:
        """Test ND matmul with batch-varying constant: C (B,m,k) @ X (B,k,n)."""
        B, m, k, n = 3, 4, 5, 6
        ...
        # Verify the result
        result = np.zeros((B, m, n))
        for b in range(B):
            result[b] = C[b] @ X.value[b]
        error = np.linalg.norm(result - target) / np.linalg.norm(target)
        assert error < 1e-4, f"Relative error {error} too large"
