...
class TestXPRESS(BaseTest):
    ...
    def test_xpress_iis_full(self) -> None:
        if cp.XPRESS in INSTALLED_SOLVERS:
            ...
            problem.solve(solver=cp.XPRESS, canon_backend=cp.SCIPY_CANON_BACKEND, **params)

            assert 'XPRESS_IIS' in problem.solution.attr
            assert 'iis_const_id' in problem.solution.attr
            self.assertCountEqual(problem.solution.attr['iis_const_id'], [constraint[0].id])
