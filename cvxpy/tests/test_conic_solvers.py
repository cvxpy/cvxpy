...
class TestIPOPT(BaseTest):

    """ Unit tests for IPOPT. """
    def setUp(self) -> None:
    ...
    def test_ipopt_parameter_update(self) -> None:
        """Test warm start.
        """
        x = cp.Variable(2)
        P = cp.Parameter(nonneg=True),
        ...
        update_parameters(P, A, b, q)
        result1 = prob.solve(solver=cp.IPOPT, nlp=True, warm_start=False)
        result2 = prob.solve(solver=cp.IPOPT, nlp=True, warm_start=True)
        self.assertAlmostEqual(result1, result2)

        update_parameters(P, A, b, q)
        result1 = prob.solve(solver=cp.IPOPT, nlp=True, warm_start=True)
        result2 = prob.solve(solver=cp.IPOPT, nlp=True, warm_start=False)
        self.assertAlmostEqual(result1, result2)
    ...
    def test_ipopt_lp_0(self) -> None:
        StandardTestLPs.test_lp_0(solver=cp.IPOPT, nlp=True)

    def test_ipopt_nonstandard_name(self) -> None:
        # Test that solver name with non-standard capitalization works.
        StandardTestLPs.test_lp_0(solver="Ipopt", nlp=True)

    def test_ipopt_lp_1(self) -> None:
        StandardTestLPs.test_lp_1(solver='IPOPT', nlp=True)

    def test_ipopt_lp_2(self) -> None:
        StandardTestLPs.test_lp_2(solver='IPOPT', nlp=True)

    def test_ipopt_lp_3(self) -> None:
        StandardTestLPs.test_lp_3(solver='IPOPT', nlp=True)

    def test_ipopt_lp_4(self) -> None:
        StandardTestLPs.test_lp_4(solver='IPOPT', nlp=True)

    def test_ipopt_lp_5(self) -> None:
        StandardTestLPs.test_lp_5(solver='IPOPT', nlp=True)

    def test_ipopt_qp_0(self) -> None:
        StandardTestQPs.test_qp_0(solver='IPOPT', nlp=True)

    def test_ipopt_qp_0_linear_obj(self) -> None:
        StandardTestQPs.test_qp_0(solver='IPOPT', nlp=True, use_quad_obj=False)

    def test_ipopt_socp_0(self) -> None:
        StandardTestSOCPs.test_socp_0(solver='IPOPT', nlp=True)

    def test_ipopt_socp_1(self) -> None:
        StandardTestSOCPs.test_socp_1(solver='IPOPT', nlp=True)

    def test_ipopt_socp_2(self) -> None:
        StandardTestSOCPs.test_socp_2(solver='IPOPT', nlp=True)

    def test_ipopt_socp_3(self) -> None:
        # axis 0
        StandardTestSOCPs.test_socp_3ax0(solver='IPOPT', nlp=True)
        # axis 1
        StandardTestSOCPs.test_socp_3ax1(solver='IPOPT', nlp=True)

    def test_ipopt_expcone_1(self) -> None:
        StandardTestECPs.test_expcone_1(solver='IPOPT', nlp=True)

    def test_ipopt_exp_soc_1(self) -> None:
        StandardTestMixedCPs.test_exp_soc_1(solver='IPOPT', nlp=True)

    def test_ipopt_pcp_0(self) -> None:
        StandardTestSOCPs.test_socp_0(solver='IPOPT', nlp=True)

    def test_ipopt_pcp_1(self) -> None:
        StandardTestSOCPs.test_socp_1(solver='IPOPT', nlp=True)

    def test_ipopt_pcp_2(self) -> None:
        StandardTestSOCPs.test_socp_2(solver='IPOPT', nlp=True)

    def test_ipopt_sdp_1min(self) -> None:
        StandardTestSDPs.test_sdp_1min(solver='IPOPT', nlp=True)

    def test_ipopt_sdp_2(self) -> None:
        # produces a different optimizer than
        # the one expected by the standard test
        places = 3
        sth = sths.sdp_2()
        sth.solve('IPOPT', nlp=True)
        sth.verify_objective(places)
        sth.check_primal_feasibility(places)
        # sth.verify_primal_values(places) # skip
        sth.check_complementarity(places)
