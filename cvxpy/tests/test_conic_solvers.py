...
@pytest.mark.skipif(
    ("CBC" not in INSTALLED_SOLVERS) or sys.platform.startswith("win"),
...
class TestHIGHS:
    ...
    def test_highs_validate_column_name(self) -> None:
        ...
        valid_names = (
            ...
            + [
                name
                for beginning, contains in zip(may_begin_with, may_contain)
            ...
            for expected_var in variables:
                if expected_var.ndim == 0:
                    ...
                    assert expected_var.name() in model, (
                        f"Expected variable {expected_var.name()} to appear in the model file."
                    )
                else:
                    # Array variables appear as name[idx] entries.
                    actual_var_count = len(
                        re.findall(re.escape(expected_var.name()) + r"\[", model)
                    )
