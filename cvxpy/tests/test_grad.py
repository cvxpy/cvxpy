...
class TestGrad(BaseTest):
    ...
    def test_pnorm(self) -> None:
        ...
        self.assertItemsAlmostEqual(
            expr.grad[self.A].toarray(), np.array([[0.6, 0], [0.8, 0], [0, -0.6], [0, 0.8]])
        )

        expr = cp.pnorm(self.A, 2, axis=1)
        self.A.value = np.array([[3, -4], [4, 3]])
        self.assertItemsAlmostEqual(
            expr.grad[self.A].toarray(), np.array([[0.6, 0], [0, 0.8], [-0.8, 0], [0, 0.6]])
        )

        expr = cp.pnorm(self.A, 2, axis=1)
        self.A.value = np.array([[0, 0], [10, 0]])
        self.assertItemsAlmostEqual(
            expr.grad[self.A].toarray(), np.array([[0, 0], [0, 1], [0, 0], [0, 0]])
        )
    ...
    def test_geo_mean(self) -> None:
        ...
        self.x.value = [1, 2]
        self.assertItemsAlmostEqual(
            expr.grad[self.x].toarray(), [np.sqrt(2) / 2, 1.0 / 2 / np.sqrt(2)]
        )
    ...
    def test_max(self) -> None:
        ...
        self.A.value = np.array([[1, 2], [4, 3]])
        self.assertItemsAlmostEqual(
            expr.grad[self.A].toarray(), np.array([[0, 0], [1, 0], [0, 0], [0, 1]])
        )

        expr = cp.max(self.A, axis=1)
        self.A.value = np.array([[1, 2], [4, 3]])
        self.assertItemsAlmostEqual(
            expr.grad[self.A].toarray(), np.array([[0, 0], [0, 1], [1, 0], [0, 0]])
        )
