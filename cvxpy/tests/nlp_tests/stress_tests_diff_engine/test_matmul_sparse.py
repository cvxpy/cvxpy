...
class TestSparseMatmulDispatch:
    ...
    def test_super_sparse_dense_routes_to_sparse(self):
        # Dense numpy operand at 1% density (< 5% threshold) must take the sparse path:
        ...
        n = 40
        full_lower = n * (n + 1) // 2
        A_sp = sp.random(n, n, density=0.01, format='csr', random_state=0)
    ...
    def test_dense_above_threshold_stays_dense(self):
        # Block-diagonal operand at 10% density (> 5% threshold) must stay on the dense
        # path. A^T A is genuinely block-sparse, so the explicit-CSR construction yields a
        ...
        A_dense = sp.block_diag(blocks).toarray()  # density = blk / n = 10%
    ...
    def test_parametric_super_sparse_not_sparsified(self):
        ...
        n = 40
        full_lower = n * (n + 1) // 2
        A_dense = sp.random(n, n, density=0.01, format='csr', random_state=0).toarray()
