...
MIP_SOLVERS = [cp.ECOS_BB, cp.GUROBI, cp.MOSEK]
...
class TestMIPVariable(BaseTest):
    ...
    def test_mip_consistency(self):
        ...
        data_recs = []
        for i in range(5):
            obj = cp.Minimize(cp.square(self.y_int - 0.2))
            p = cp.Problem(obj, [self.A_bool == 0, self.x_bool == self.B_int])
            data_recs.append(p.get_problem_data(cp.ECOS_BB))
    ...
    def test_all_solvers(self):
        for solver in self.solvers:
            ...
            if solver in [cp.CPLEX, cp.GUROBI, cp.MOSEK, cp.XPRESS, cp.ECOS_BB]:
