...
class TestAtoms(BaseTest):
    ...
    def test_kron_canon_1(self) -> None:
    ...
    def test_kron_canon_2(self) -> None:
        #
        # Test case 3
        #
        for c_dims in [(1, 2), (2, 1), (2, 2), (3, 5), (5, 3)]:
            print(c_dims)
            Z = cp.Variable(shape=(2, 3))
            np.random.seed(0)
            C = np.random.rand(*c_dims)
            L = np.random.rand(*Z.shape)
            U = L + np.random.rand(*Z.shape)
            kronZC = cp.kron(Z, C)
            constraints = [np.kron(U, C) >= kronZC, kronZC >= np.kron(L, C)]
            obj_expr = cp.sum(Z)
