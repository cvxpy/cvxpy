...
class TestDgp2Dcp(BaseTest):
    ...
    def test_prod(self) -> None:
        ...
        np.testing.assert_almost_equal(
            np.prod(X, axis=0, keepdims=True), cvxpy.prod(X, axis=0, keepdims=True).value
        )
        np.testing.assert_almost_equal(
            np.prod(X, axis=1, keepdims=True), cvxpy.prod(X, axis=1, keepdims=True).value
        )
        ...
        np.testing.assert_almost_equal(np.prod(X), cvxpy.prod(X).value)
        np.testing.assert_almost_equal(
            np.prod(X, keepdims=True), cvxpy.prod(X, keepdims=True).value
        )
    ...
    def test_sum_largest(self) -> None:
        ...
        obj = cvxpy.Minimize(
            cvxpy.sum_largest(
                cvxpy.hstack(
                    [3 * x[0] ** 0.5 * x[1] ** 0.5, x[0] * x[1] + 0.5 * x[1] * x[3] ** 3, x[2]]
                ),
                2,
            )
        )
    ...
    def test_paper_example_sum_largest(self) -> None:
        ...
        obj = cvxpy.Minimize(
            cvxpy.sum_largest(
                cvxpy.hstack([3 * x0**0.5 * x1**0.5, x0 * x1 + 0.5 * x1 * x3**3, x2]), 2
            )
        )
        constr = [x0 * x1 * x2 >= 16]
    ...
    def test_paper_example_eye_minus_inv(self) -> None:
        ...
        constr = [
            cvxpy.geo_mean(cvxpy.diag(X)) == 0.1,
    ...
    def test_sum_vector(self) -> None:
        ...
        problem = cvxpy.Problem(
            cvxpy.Minimize(cvxpy.sum(h)), [cvxpy.multiply(w, h) >= 10, cvxpy.sum(w) <= 10]
        )
        problem.solve(SOLVER, gp=True)
    ...
    def test_sum_squares_vector(self) -> None:
        ...
        h = cvxpy.Variable(2, pos=True)
        problem = cvxpy.Problem(
            cvxpy.Minimize(cvxpy.sum_squares(h)), [cvxpy.multiply(w, h) >= 10, cvxpy.sum(w) <= 10]
        )
        problem.solve(SOLVER, gp=True)
    ...
    def test_sum_matrix(self) -> None:
        ...
        problem = cvxpy.Problem(
            cvxpy.Minimize(cvxpy.sum(h)), [cvxpy.multiply(w, h) >= 10, cvxpy.sum(w) <= 20]
        )
        problem.solve(SOLVER, gp=True)
        np.testing.assert_almost_equal(problem.value, 8, decimal=3)
