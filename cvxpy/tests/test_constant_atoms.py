...
v_np = np.array([-1.0, 2, -2]).T

# Defined here to be used in KNOWN_SOLVER_ERRORS


def log_sum_exp_axis_0(x):
    return cp.log_sum_exp(x, axis=0, keepdims=True)  # noqa E371
...
atoms_minimize = [
    ...
    (lambda x: cp.cumsum(x, axis=1), (2, 2), [[[-5, 2], [-3, 1]]], Constant([[-5, 2], [-8, 3]])),
    (lambda x: cp.cumsum(x, axis=0), (2, 2), [[[-5, 2], [-3, 1]]], Constant([[-5, -3], [-3, -2]])),
    (lambda x: cp.cummax(x, axis=1), (2, 2), [[[-5, 2], [-3, 1]]], Constant([[-5, 2], [-3, 2]])),
    (lambda x: cp.cummax(x, axis=0), (2, 2), [[[-5, 2], [-3, 1]]], Constant([[-5, 2], [-3, 1]])),
    (cp.diag, (2,), [[[-5, 2], [-3, 1]]], Constant([-5, 1])),
    (cp.diag, (2, 2), [[-5, 1]], Constant([[-5, 0], [0, 1]])),
    (cp.exp, (2, 2), [[[1, 0], [2, -1]]], Constant([[math.e, 1], [math.e**2, 1.0 / math.e]])),
    (
        lambda x: cp.xexp(cp.pos(x)),
        (2, 2),
        [[[1, 3], [2, 0.5]]],
        Constant([[math.e, 3 * math.e**3], [2 * math.e**2, 0.5 * math.e**0.5]]),
    ),
    (cp.huber, (2, 2), [[[0.5, -1.5], [4, 0]]], Constant([[0.25, 2], [7, 0]])),
    (
        lambda x: cp.huber(x, 2.5),
        (2, 2),
        [[[0.5, -1.5], [4, 0]]],
        Constant([[0.25, 2.25], [13.75, 0]]),
    ),
    (cp.inv_pos, (2, 2), [[[1, 2], [3, 4]]], Constant([[1, 1.0 / 2], [1.0 / 3, 1.0 / 4]])),
    (
        lambda x: (x + Constant(0)) ** -1,
        (2, 2),
        [[[1, 2], [3, 4]]],
    ...
    (
        lambda x: cp.kron(np.array([[1, 2], [3, 4]]), x),
        (4, 4),
        [np.array([[5, 6], [7, 8]])],
        Constant(np.kron(np.array([[1, 2], [3, 4]]), np.array([[5, 6], [7, 8]]))),
    ),
    (
        lambda x: cp.kron(np.array([[1, 2], [3, 4], [5, 6]]), x),
        (6, 4),
        [np.array([[5, 6], [7, 8]])],
    ...
    (
        lambda x: cp.kron(x, np.array([[1, 2], [3, 4]])),
        (4, 4),
        [np.array([[5, 6], [7, 8]])],
        Constant(np.kron(np.array([[5, 6], [7, 8]]), np.array([[1, 2], [3, 4]]))),
    ),
    (
        lambda x: cp.kron(x, np.array([[1, 2], [3, 4], [5, 6]])),
        (6, 4),
        [np.array([[5, 6], [7, 8]])],
    ...
    (
        cp.logistic,
        (2, 2),
        [[[math.log(5), math.log(7)], [0, math.log(0.3)]]],
        Constant([[math.log(6), math.log(8)], [math.log(2), math.log(1.3)]]),
    ),
    (cp.matrix_frac, tuple(), [[1, 2, 3], [[1, 0, 0], [0, 1, 0], [0, 0, 1]]], Constant([14])),
    (
        cp.matrix_frac,
        tuple(),
        [[1, 2, 3], [[67, 78, 90], [78, 94, 108], [90, 108, 127]]],
        Constant([0.46557377049180271]),
    ),
    (
        cp.matrix_frac,
        tuple(),
        [[[1, 2, 3], [4, 5, 6]], [[67, 78, 90], [78, 94, 108], [90, 108, 127]]],
        Constant([0.768852459016]),
    ),
    (cp.maximum, (2,), [[-5, 2], [-3, 1], 0, [-1, 2]], Constant([0, 2])),
    (cp.maximum, (2, 2), [[[-5, 2], [-3, 1]], 0, [[5, 4], [-1, 2]]], Constant([[5, 4], [0, 2]])),
    (cp.max, tuple(), [[[-5, 2], [-3, 1]]], Constant([2])),
    (cp.max, tuple(), [[-5, -10]], Constant([-5])),
    (
        lambda x: cp.max(x, axis=0, keepdims=True),
        (1, 2),
        [[[-5, 2], [-3, 1]]],
        Constant([[2], [1]]),
    ),
    ...
    (
        lambda x: cp.dotsort(cp.abs(x), np.array([2, 1, 0.1])),
        tuple(),
        [[1, 2, 3, -4, -5]],
        Constant([10 + 4 + 0.3]),
    ),
    ...
    (lambda x: cp.pnorm(x, 2, axis=0), (2,), [[[1, 2], [3, 4]]], Constant([math.sqrt(5), 5.0]).T),
    (
        lambda x: cp.pnorm(x, 2, axis=1),
        (2,),
        [[[1, 2], [4, 5]]],
    ...
    (lambda x: cp.power(x, -0.7), tuple(), [7.45], Constant([0.24518314363015764])),
    ...
    (
        lambda x: x[np.array([[3, 4, 5], [6, 7, 8]]).T % 2 == 0],
        (2,),
        [[[3, 4, 5], [6, 7, 8]]],
        Constant([6, 4, 8]),
    ),
...
atoms_maximize = [
    (
        cp.entr,
        (2, 2),
        [[[1, math.e], [math.e**2, 1.0 / math.e]]],
        Constant([[0, -math.e], [-2 * math.e**2, 1.0 / math.e]]),
    ),
    (
        cp.log_det,
        tuple(),
        [[[20, 8, 5, 2], [8, 16, 2, 4], [5, 2, 5, 2], [2, 4, 2, 4]]],
        Constant([7.7424020218157814]),
    ),
    ...
    (
        lambda x: cp.geo_mean(x, [0.4, 0.8, 4.9]),
        tuple(),
        [[0.5, 1.8, 17]],
    ...
    (cp.diff, (3,), [[2.1, 1, 4.5, -0.1]], Constant([-1.1, 3.5, -4.6])),
    (lambda x: cp.diff(x, 2), (2,), [[2.1, 1, 4.5, -0.1]], Constant([4.6, -8.1])),
    (lambda x: cp.diff(x, 1, axis=0), (1, 2), [np.array([[-5, -3], [2, 1]])], Constant([[7], [4]])),
    (lambda x: cp.diff(x, 1, axis=1), (2, 1), [np.array([[-5, -3], [2, 1]])], Constant([[2, -1]])),
    (lambda x: cp.pnorm(x, 0.5), tuple(), [[1.1, 2, 0.1]], Constant([7.724231543909264])),
    (lambda x: cp.pnorm(x, -0.4), tuple(), [[1.1, 2, 0.1]], Constant([0.02713620334])),
    (lambda x: cp.pnorm(x, -1), tuple(), [[1.1, 2, 0.1]], Constant([0.0876494023904])),
    (lambda x: cp.pnorm(x, -2.3), tuple(), [[1.1, 2, 0.1]], Constant([0.099781528576])),
    (cp.lambda_min, tuple(), [[[2, 0], [0, 1]]], Constant([1])),
    (cp.lambda_min, tuple(), [[[5, 7], [7, -3]]], Constant([-7.06225775])),
    (
        lambda x: cp.lambda_sum_smallest(x, 2),
        tuple(),
        [[[1, 2, 3], [2, 4, 5], [3, 5, 6]]],
        Constant([-0.34481428]),
    ),
    ...
    (
        cp.log1p,
        (2, 2),
        [[[0, math.e - 1], [math.e**2 - 1, 1.0 / math.e - 1]]],
        Constant([[0, 1], [2, -1]]),
    ),
    ...
    (
        lambda x: cp.sum_smallest(x, 4),
        tuple(),
        [[[-3, -4, 5], [6, 7, 8], [9, 10, 11]]],
        Constant([-3 - 4 + 5 + 6]),
    ),
    (
        lambda x: (x + Constant(0)) ** 0.5,
        (2, 2),
        [[[2, 4], [16, 1]]],
