...
import cvxpy as cp
import cvxpy.constraints as constraints
...
class TestComplex(BaseTest):
    ...
    def test_duals(self) -> None:
        ...
        #
        ####################################
        con_a = constraints.PowCone3D(y[0], y[1], y[2], [0.25])
        helper_prob_a = cp.Problem(helper_objective, [con_a])
        helper_prob_a.solve()
        expect_dual_a = con_a.dual_value

        con_b = constraints.ExpCone(y[0], y[1], y[2])
    ...
    def test_illegal_complex_args(self) -> None:
        ...
        with self.assertRaises(ValueError):
            constraints.ExpCone(x[0], x[1], x[2])
        with self.assertRaises(ValueError):
            constraints.PowCone3D(x[0], x[1], x[2], [0.5])
        with self.assertRaises(ValueError):
            constraints.NonNeg(x)
        with self.assertRaises(ValueError):
            cp.NonPos(x)
