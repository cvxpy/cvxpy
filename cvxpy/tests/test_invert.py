...
# alpha3 = runif_in_simplex(dims - 1)
...
# print(x_power1.value, x_power2.value, x_power3.value)
...
if axis == 1:
    W = W.T
    alpha = alpha.T
pow_con = cp.PowConeND(W, hypos, alpha, axis=axis)
cons = [
    x[0] + x[1] + 0.5 * x[2] == 2,
...
sth = STH((obj, None), [(x, None), (hypos, None)], [(con, None) for con in cons])
sth.check_stationary_lagrangian(4)
