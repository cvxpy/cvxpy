...
class TestCvxcoreIssues(BaseTest):
    ...
    def test_issue_1132_sdp_kron_segfault(self) -> None:
        ...
        # The scaled-down SDP is marginal for CLARABEL: identical stuffed data
        # can terminate at reduced accuracy depending on machine load, so
        # accept both statuses and assert parity with the default path.
        self.assertIn(prob_de.status, (cp.OPTIMAL, cp.OPTIMAL_INACCURATE))

        prob_base = build()
        prob_base.solve(solver=SOLVER)
        self.assertAlmostEqual(prob_de.value, prob_base.value, places=2)
