...
class Problem(object):
    ...
    def filter_constraints(self, constraints):
        ...
        nonlinear_constraints = [c for c in constraints if isinstance(c, NonlinearConstraint)]
        return (eq_constraints, ineq_constraints, soc_constraints, nonlinear_constraints)
    ...
    def canonicalize(self):
        ...
        eq_constr, ineq_constr, soc_constr, nonlin_constr = self.filter_constraints(constraints)
        ...
        dims['s'] = []
        return (obj,eq_constr,ineq_constr,nonlin_constr,dims)
    ...
    def _solve(self, solver=s.ECOS, ignore_dcp=False):
        ...
        objective,eq_constr,ineq_constr,nonlin_constr, dims = self.canonicalize()
        ...
        F = self.nonlinear_constraint_function(nonlin_constr, var_ids, self.interface)
        ...
        if status == s.SOLVED:
            ...
            if nonlin_constr:
                self.save_values(results['zl'], ineq_constr)
            else:
    ...
    def nonlinear_constraint_function(self, nl_funcs, var_ids, interface):
        ...
        # loop through all var_ids, collecting only the vars involved
        # really inefficient, but we have to do this since variables get
        # scalarized before stuffing matrices...
        ...
        for func in nl_funcs:
            ...
            for variable in func.vars_involved:
                coefficients = variable.coefficients(interface)
                for id in coefficients.keys():
                    if id in var_ids:
                        indices.append(var_ids[id])   # collect all the indices
            big_x[indices] = x0
