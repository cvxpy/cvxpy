...
def build_non_disciplined_error_msg(problem, discipline_type) -> str:
    ...
    def find_non_prop_leaves(expr, res=None):
        ...
        if len(expr.args) == 0 and getattr(expr, prop_name)():
            return res
        if (not getattr(expr, prop_name)()) and all(
            getattr(child, prop_name)() for child in expr.args
        ):
            str_expr = str(expr)
    ...
    if not getattr(problem.objective, prop_name)():
        ...
        if len(non_disciplined_leaves) > 0:
            ...
            msg = (
                "The objective is not {}, even though each sub-expression is.\n"
                "You are trying to {} a function that is {}."
            ).format(
                discipline_type,
