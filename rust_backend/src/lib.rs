...
fn build_matrix(
    ...
    let mut offset = 0i64;
    let tensor = TensorRepresentation::combine(
        linops
            .into_iter()
            .map(|linop| {
...
fn transform_tensor_to_mat(
    ctx: &ViewContext,
    tensor: TensorRepresentation,
    offset: u64,
) -> (Vec<f64>, (Vec<u64>, Vec<u64>), (u64, u64)) {
    let rows = tensor
        .col
        .into_iter()
        ...
        .collect();
    let shape = (
        offset * u64::try_from(ctx.var_length + 1).unwrap(),
        ctx.param_size_plus_one.try_into().unwrap(),
    );
